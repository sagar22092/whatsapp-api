...
/* ───────── GET QR ───────── */
export async function getQR(userId, sessionId, timeout = 10000) {
  let client = await getClient(userId, sessionId);

  if (client.connected) return null;

  if (!client.qr) {
    await destroyClient(userId, sessionId, false);
    client = await initSession(userId, sessionId);
  }

  await Promise.race([
    client.qrReady,
    new Promise((_, r) =>
      setTimeout(() => r(new Error("QR timeout")), timeout),
    ),
  ]);
...
export async function getPairCode(
  ...
  if (!client.sock?.requestPairingCode) {
    await destroyClient(userId, sessionId, false);
    client = await initSession(userId, sessionId);
  }
  ...
  const code = await Promise.race([
    ...
    new Promise((_, r) =>
      setTimeout(() => r(new Error("Pair code timeout")), timeout),
    ),
  ]);
